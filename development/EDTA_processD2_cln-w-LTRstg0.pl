...
my $cleanup_gff = "$script_path/bin/filter_gff_TIR.pl";
...
sub FindLTR() {
	my $file = $_[0];
	`$genometools suffixerator -db $file -indexname $file -tis -suf -lcp -des -ssp -sds -dna -mirrored`;
	`$genometools ltrharvest -minlenltr 100 -maxlenltr 7000 -mintsd 4 -maxtsd 6 -motif TGCA -motifmis 1 -similar 85 -vic 10 -seed 20 -seqids yes -index $file -out $file.LTR 2>/dev/null`;
...
#	`cat $file.HelitronScanner.draw.hel.fa $file.HelitronScanner.draw.rc.hel.fa | perl -nle \'print \$_ and next unless /^>/; my \$line=(split)[0]; \$line=~s/\#SUB_//; \$line=~s/[\#\\/]/_/g; print \"\$line\#DNA\/Helitron\"\' > $file.Helitron`;
	`cat $file.HelitronScanner.filtered.fa $script_path/database/HelitronScanner.training.set.fa > $file.Helitron`;
	}
...
`ln -s ../$genome.EDTA.LTR/$genome.LTR.fa.stg0 $genome.LTR.fa.stg0` unless -s "$genome.LTR.fa.stg0";
...
`cat $genome.LTR.fa.stg0 $genome.TIR.fa.stg0.LTR $genome.TIR.fa.stg0.Helitron > $genome.TIR.fa.stg0.LTR.Helitron`;
...
`cat $genome.LTR.fa.stg0 $genome.Helitron.fa.stg0.LTR $genome.Helitron.fa.stg0.TIR > $genome.Helitron.fa.stg0.LTR.TIR`;
...
for (my $i=0; $i<$maxit; $i++){
	...
	&RMclean("$genome.LTR.Helitron.fa.HQ$i", "$genome.TIR.fa.stg0", 80, 0);
...
`cat $genome.LTR.fa.stg0 $genome.TIR.fa.HQ$maxit $genome.Helitron.fa.HQ$maxit | perl -nle 's/>/\\n>/g unless /^>/; print \$_' > $genome.LTR.TIR.Helitron.fa.stg1.raw`;
