...
`mkdir $genome.EDTA.combineH3` unless -e "$genome.EDTA.combineH3" && -d "$genome.EDTA.combineH3"; #test line

# enter the combine folder for EDTA processing
#chdir "$genome.EDTA.combine";
chdir "$genome.EDTA.combineH3";
...
`cat $genome.LTR.raw.fa $genome.TIR.raw.fa.HQ $genome.Helitron.raw.fa.HQ | perl -nle 's/>/\\n>/g unless /^>/; print \$_' > $genome.LTR.TIR.Helitron.fa.stg1.raw`;
`perl $cleanup_nested -in $genome.LTR.TIR.Helitron.fa.stg1.raw -threads $threads -minlen 80 -cov 0.95 -blastplus $blast`;
