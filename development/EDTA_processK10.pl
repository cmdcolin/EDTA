...
my $LTRint = "$genome.LTR.intact.fa";
...
# purify TIR
&Purifier("$TIR", "$LTRint", $mindiff_TIR);
&Purifier("$TIR.HQ", "$HEL", $mindiff_TIR);
`mv $TIR.HQ.HQ $TIR.HQ`;

# purify Helitron
&Purifier("$HEL", "$TIR.HQ", $mindiff_HEL);
&Purifier("$HEL.HQ", "$LTRint", $mindiff_HEL);
`mv $HEL.HQ.HQ $HEL.HQ`;
...
if (1){
# clean LTRs and LINEs in Helitrons
if (-s "$LTR.cln"){
	`cat $LTR.cln $LINE > $genome.LINE_LTR.raw.fa`;
	$err = `${repeatmasker}RepeatMasker -e ncbi -pa $threads -q -no_is -norna -nolow -div 40 -lib $genome.LINE_LTR.raw.fa $HEL.HQ 2>&1`;
	if ($err !~ /done/) {
		`ln -s $LTR $LTR.masked` if $err =~ s/^.*(No repetitive sequences were detected.*)\s+$/Warning: $1/s;
		print STDERR "\n$err\n";
		}
	`perl $cleanup_tandem -misschar N -nc 50000 -nr 0.9 -minlen 80 -minscore 3000 -trf 0 -cleanN 1 -cleanT 1 -f $HEL.HQ.masked > $HEL.cln`;
	} else {
		`cp $HEL.HQ $HEL.cln`;
	}
}

# re-purify TIR with clean Helirons
&Purifier("$TIR", "$LTRint", $mindiff_TIR);
&Purifier("$TIR.HQ", "$HEL.cln", $mindiff_TIR);
...
`cat $TIR.cln $HEL.HQ | perl -nle 's/>/\\n>/g unless /^>/; print \$_' > $genome.TIR.Helitron.fa.stg1.raw`;
#`cat $TIR.HQ $HEL.cln | perl -nle 's/>/\\n>/g unless /^>/; print \$_' > $genome.TIR.Helitron.fa.stg1.raw`;
