...
#`cat $LTR $genome.nonLTR.raw.fa.cln $genome.TIR.Helitron.fa.stg1.raw.cln.cln > $genome.EDTA.stg1.raw.cln`;
...
if (0){
`perl $cleanup_proteins -seq $genome.EDTA.stg1.raw.cln -rmdnate 0 -rmline 1 -rmprot 1 -protlib $protlib -blast $blast -threads $threads`;
`perl $rename_TE $genome.EDTA.stg1.raw.cln.clean > $genome.EDTA.fa.stg1`;
}
`cp $genome.EDTA.stg1.raw.cln $genome.EDTA.fa.stg1`;
