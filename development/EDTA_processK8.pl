...
if (-s "$SINE"){
	`cat $LTR.cln $LINE $TIR.HQ > $genome.LINE_LTR_TIR.raw.fa`;
	$err = `${repeatmasker}RepeatMasker -e ncbi -pa $threads -q -no_is -norna -nolow -div 40 -lib $genome.LINE_LTR_TIR.raw.fa $SINE 2>&1`;
	if ($err !~ /done/) {
