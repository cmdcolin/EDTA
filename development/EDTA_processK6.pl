...
sub Purger() {
	my ($TE1, $TE2) = ($_[0], $_[1]);
	`ln -s $TE1 $TE1-$TE2`;
	$err = `${repeatmasker}RepeatMasker -e ncbi -pa $rm_threads -q -no_is -norna -nolow -div 40 -lib $TE1-$TE2 $TE2 2>&1`;
        if ($err !~ /done/) {
                `cp $TE1-$TE2 $TE1-$TE2.masked` if $err =~ s/^.*(No repetitive sequences were detected.*)\s+$/Warning: $1/s;
                print STDERR "\n$err\n";
                }
        `perl $cleanup_tandem -misschar N -nc 50000 -nr 0.9 -minlen 80 -minscore 3000 -trf 0 -cleanN 1 -cleanT 1 -f $TE1-$TE2.masked > $TE1.cln`;
        }
...
&Purifier("$HEL.cln", "$TIR", 1); #this line maybe overkill
	#&Purifier("$HEL_TM.HQ", "$TIR", 1);
	#`cat "$HEL.HQ.HQ" "$HEL_TM.HQ.HQ" > "$HEL.HQ"`;
	#`cat "$HEL.HQ.HQ" "$HEL_TM.HQ" > "$HEL.HQ"`;
`cat "$HEL.cln.HQ" "$HEL_TM.cln" > "$HEL.HQ"`;
...
# purify TIR
&Purifier("$TIR", "$LTR.HQ", $mindiff_TIR);
...
`cat $HEL.HQ $TIR.HQ > $genome.TIR.Helitron.fa.stg1.raw.cln`;
...
`cat $LTR $genome.nonLTR.raw.fa $genome.TIR.Helitron.fa.stg1.raw.cln.cln > $genome.EDTA.fa.stg1`;
